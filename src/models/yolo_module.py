...
class YOLOModule(L.LightningModule):
    ...
    def model_step(
        ...
        x, y = batch
        x = self.forward(x)
        logits = torch.randn(1, 10, device=x.device)
        loss = torch.abs(torch.zeros(1, device=x.device) + x.sum())
        preds = torch.argmax(logits, dim=1)
        target = torch.randint(0, 10, (1,), device=x.device)
